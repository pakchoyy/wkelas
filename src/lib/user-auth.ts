...
import { cloudPilotMessage, isCloudPilotAllowed } from '../shared/cloud-access'
...
export function useUserSession(client: SupabaseClient | null) {
  ...
  useEffect(() => {
    ...
    const apply=(next:Session|null) => {
      if(cancelled)return
      if(next?.user && !isCloudPilotAllowed(next.user.email)) {
        setSession(null)
        if (!isDemoMode()) useAuthStore.getState().logout()
...
export async function signInUser(client:SupabaseClient,email:string,password:string) { if(!isCloudPilotAllowed(email)) throw new Error(cloudPilotMessage()); const {error}=await client.auth.signInWithPassword({email:email.trim(),password}); if(error) throw new Error('Email atau kata sandi belum sesuai. Periksa kembali lalu coba lagi.') }
export async function signUpUser(client:SupabaseClient,email:string,password:string) { if(!isCloudPilotAllowed(email)) throw new Error(cloudPilotMessage()); const {data,error}=await client.auth.signUp({email:email.trim(),password}); if(error) throw new Error(error.message.includes('already registered')?'Email sudah terdaftar. Pilih Masuk.':'Pendaftaran belum berhasil. Periksa email dan kata sandi lalu coba lagi.'); return data.session }
...
export async function resetUserPassword(client:SupabaseClient,email:string) { if(!isCloudPilotAllowed(email)) throw new Error(cloudPilotMessage()); const {error}=await client.auth.resetPasswordForEmail(email.trim(),{redirectTo:`${window.location.origin}/#/login`}); if(error) throw new Error('Email reset kata sandi belum dapat dikirim. Periksa email dan konfigurasi Supabase.') }
