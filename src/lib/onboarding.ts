...
export async function saveInitialClass(db: BgyDatabase, data: SetupData, supabaseUid = 'local', skip = false) {
  ...
  return db.transaction('rw', [db.guru, db.kelas, db.mata_pelajaran, db.pengaturan], async () => {
    ...
    const guruId = await db.guru.add({ supabase_uid: supabaseUid, nama: setup.namaWali, email: 'admin@lokal', nip: setup.nip, nama_sekolah: setup.namaSekolah, tahun_ajaran_aktif: setup.tahunAjaran, semester_aktif: setup.semester, created_at: timestamp, updated_at: timestamp })
