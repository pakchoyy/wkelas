...
export function schoolDayStatus(date: string, schoolDays: number, events: CalendarEvent[]) {
  ...
  const extra = events.some(event => event.jenis === 'pengganti' && date >= event.tanggal_mulai && date <= (event.tanggal_selesai || event.tanggal_mulai))
  const active = (Number.isFinite(day) && day >= 1 && day <= (schoolDays === 6 ? 6 : 5) && !holiday) || extra
