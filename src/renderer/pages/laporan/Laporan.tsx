...
export default function Laporan() {
  ...
  const [identity,setIdentity]=useState({sekolah:'-',kelas:'-',semester:'-',tahun:'-',guru:'-',logo:''})
  ...
  return (
    <div><style>{`@media print{aside,header,button,.no-print{display:none!important}main{overflow:visible!important;padding:0!important}.report{box-shadow:none!important;border:1px solid #999!important}.report .overflow-x-auto{overflow:visible!important}.report table{min-width:0!important;font-size:9px!important}.report th,.report td{padding:5px!important}body{background:white!important}}`}</style>
      ...
      <div className="report mb-4 rounded-xl border border-slate-200 bg-white p-4"><div className="flex items-center gap-4">{identity.logo&&<img src={identity.logo} alt="" className="size-20 shrink-0 rounded-xl object-contain ring-1 ring-slate-200"/>}<div className={`min-w-0 flex-1 text-center ${identity.logo ? 'pr-0 sm:pr-20' : ''}`}><h1 className="font-extrabold uppercase">Laporan {tabs.find(item=>item.id===tab)?.label}</h1><p className="mt-1 text-sm font-semibold">{identity.sekolah}</p><p className="mt-1 text-xs text-slate-500">{identity.kelas} · Semester {identity.semester} · {identity.tahun} · Wali Kelas: {identity.guru}</p></div></div></div>
      ...
      {exportMessage && <p role={exportMessage.ok ? 'status' : 'alert'} className={`no-print mb-3 rounded-xl border p-3 text-sm font-semibold ${exportMessage.ok?'border-emerald-200 bg-emerald-50 text-emerald-800':'border-red-200 bg-red-50 text-red-700'}`}>{exportMessage.text}</p>}
