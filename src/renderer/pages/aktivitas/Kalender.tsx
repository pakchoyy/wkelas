...
export default function Kalender() {
  ...
  useEffect(() => { load(); Promise.all([db.kelas.get(kelasId), db.pengaturan.get(`presensi_${kelasId}`)]).then(([kelas, setting]) => { let cfg:any={}; if(setting?.value) try{cfg=JSON.parse(setting.value)}catch{}; const sem=kelas?.semester||1; setSemester(sem); setPeriod({ mulai: sem===1 ? cfg.s1Mulai||'2026-07-01' : cfg.s2Mulai||'2027-01-01', akhir: sem===1 ? cfg.s1Akhir||'2026-12-31' : cfg.s2Akhir||'2027-06-30', hariSekolah: cfg.hariSekolah||5 }) }) }, [kelasId])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setShowForm(false)
    setForm({ tanggal_mulai: todayISO(), tanggal_selesai: '', judul: '', jenis: 'kegiatan', deskripsi: '' });setEditId(null)
    await load();setToast(editId?'Kegiatan berhasil diperbarui':'Kegiatan berhasil ditambahkan')
  }
  const savePeriod=async()=>{const item=await db.pengaturan.get(`presensi_${kelasId}`);let cfg:any={};if(item?.value)try{cfg=JSON.parse(item.value)}catch{};await db.pengaturan.put({key:`presensi_${kelasId}`,value:JSON.stringify({...cfg,hariSekolah:period.hariSekolah,[semester===1?'s1Mulai':'s2Mulai']:period.mulai,[semester===1?'s1Akhir':'s2Akhir']:period.akhir}),updated_at:new Date().toISOString()});setToast('Periode akademik berhasil disimpan')}
  const openEdit=(item:KalenderAkademik)=>{setEditId(item.id);setForm({tanggal_mulai:item.tanggal_mulai,tanggal_selesai:item.tanggal_selesai||'',judul:item.judul,jenis:item.jenis,deskripsi:item.deskripsi||''});setShowForm(true)}
  const remove=async(item:KalenderAkademik)=>{if(!window.confirm(`Hapus ${item.judul}?`))return;await window.electronAPI.kalender.delete(item.id);await load();setToast('Kegiatan berhasil dihapus')}

  return (
    <div>{toast&&<div className="fixed left-1/2 top-20 z-[100] -translate-x-1/2 rounded-xl bg-emerald-600 px-5 py-3 text-sm font-bold text-white shadow-xl">{toast}</div>}
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xl font-bold">Kalender Akademik</h2>
        <button onClick={() => {setEditId(null);setForm({ tanggal_mulai: todayISO(), tanggal_selesai: '', judul: '', jenis: 'kegiatan', deskripsi: '' });setShowForm(true)}} className="flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-semibold text-white" style={{ background: 'linear-gradient(135deg, #0ea5a0, #0d7a8a)' }}><Plus size={16} /> Tambah</button>
      </div>

      <div className="grid gap-4 mb-5 lg:grid-cols-[1fr_260px]"><div className="rounded-2xl border border-slate-200 bg-white p-5"><div className="mb-4 flex items-center gap-2 font-bold"><CalendarDays size={18} className="text-emerald-600"/>Batas Waktu Semester</div><div className="grid gap-3 md:grid-cols-3"><label className="text-xs font-bold text-slate-500">Mulai Semester<input type="date" value={period.mulai} onChange={(e)=>setPeriod({...period,mulai:e.target.value})} className="field mt-1.5"/></label><label className="text-xs font-bold text-slate-500">Akhir Semester<input type="date" value={period.akhir} onChange={(e)=>setPeriod({...period,akhir:e.target.value})} className="field mt-1.5"/></label><label className="text-xs font-bold text-slate-500">Sistem Hari Sekolah<select value={period.hariSekolah} onChange={(e)=>setPeriod({...period,hariSekolah:Number(e.target.value)})} className="field mt-1.5"><option value={5}>Senin–Jumat</option><option value={6}>Senin–Sabtu</option></select></label></div><div className="mt-3 flex items-center justify-between"><p className="text-xs text-slate-400">Periode ini digunakan oleh Presensi, Perilaku, Rencana, dan Jurnal.</p><button onClick={savePeriod} className="flex items-center gap-2 rounded-lg bg-emerald-600 px-3 py-2 text-xs font-bold text-white"><Save size={14}/>Simpan Periode</button></div></div><div className="rounded-2xl bg-indigo-900 p-5 text-white"><div className="text-xs font-bold uppercase tracking-wider text-emerald-300">Hari Efektif Belajar</div><div className="mt-4 text-4xl font-extrabold">{effectiveDays}</div><div className="mt-1 text-xs text-indigo-200">hari setelah akhir pekan dan hari libur</div></div></div>
          ...
          <div key={item.id} className="rounded-xl p-4 flex items-start gap-3" style={{ background: 'var(--card-bg)', boxShadow: 'var(--shadow)' }}>
            ...
            <div className="flex-1">
              <div className="flex items-start justify-between">
                ...
                <div className="flex"><button onClick={()=>openEdit(item)} className="p-1.5 text-slate-400 hover:text-blue-600"><Pencil size={14}/></button><button onClick={()=>remove(item)} className="p-1.5 text-slate-400 hover:text-red-600"><Trash2 size={14}/></button></div>
              </div>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl" style={{ background: 'var(--card-bg)', boxShadow: 'var(--shadow-lg)' }}>
            <div className="flex items-center justify-between px-4 py-3 border-b"><h3 className="text-sm font-bold">{editId?'Edit Kegiatan':'Tambah Kegiatan'}</h3><button onClick={() => setShowForm(false)} className="p-1 hover:bg-gray-100 rounded-lg">✕</button></div>
