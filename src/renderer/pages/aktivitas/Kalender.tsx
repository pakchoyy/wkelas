...
const JENIS_LABEL: Record<string, string> = { libur_nasional: 'Libur Nasional', libur_sekolah: 'Libur Sekolah', kts: 'KTS / Tengah Semester', kpp: 'Kegiatan Puasa', pengganti: 'Hari Pengganti' }
...
function KalenderKelas({kelasId}: {kelasId:number}) {
  ...
  return (
    <div>{error && <p role="alert" className="mb-3 text-red-700">{error}</p>}{!ready && !error && <p role="status">Memuat kalender...</p>}<fieldset disabled={busy || !ready} className="min-w-0">{toast&&<div className="fixed left-1/2 top-20 w-[calc(100%_-_2rem)] max-w-md z-[100] -translate-x-1/2 rounded-xl bg-emerald-600 px-5 py-3 text-sm font-bold text-white shadow-xl">{toast}</div>}
      ...
      <div className="mb-4 rounded-xl border border-sky-200 bg-sky-50 px-3 py-2.5 text-xs leading-5 text-sky-900"><p className="flex items-start gap-2 font-semibold"><Info size={16} className="mt-0.5 shrink-0 text-sky-600"/>Pilih tanggal di kalender untuk melihat agenda, mengedit agenda, atau menambahkan agenda baru.</p><p className="mt-1 pl-6 text-sky-800">Libur Nasional/Sekolah membuat presensi tidak muncul. KTS dan Kegiatan Puasa tetap masuk sekolah, tetapi tidak dihitung hari efektif. Hari Pengganti dihitung sebagai hari belajar tambahan.</p></div>


      <div className="grid gap-4 mb-5 lg:grid-cols-[1fr_260px]"><div className="rounded-2xl border border-slate-200 bg-white p-5"><div className="mb-4 flex flex-wrap items-center gap-2 font-bold"><span className="inline-flex items-center gap-2"><CalendarDays size={18} className="text-emerald-600"/>Batas Waktu Semester</span><span className="ml-auto flex rounded-xl bg-slate-100 p-1" role="group" aria-label="Pilih semester"><button aria-pressed={semester===1} onClick={()=>switchSemester(1)} className={`min-h-9 rounded-lg px-3 text-xs font-bold ${semester===1?'bg-white text-teal-700 shadow-sm':'text-slate-500'}`}>Ganjil</button><button aria-pressed={semester===2} onClick={()=>switchSemester(2)} className={`min-h-9 rounded-lg px-3 text-xs font-bold ${semester===2?'bg-white text-teal-700 shadow-sm':'text-slate-500'}`}>Genap</button></span></div><div className="grid gap-3 md:grid-cols-3"><label className="text-xs font-bold text-slate-500">Mulai Semester<input type="date" value={period.mulai} onChange={(e)=>setPeriod({...period,mulai:e.target.value})} className="field mt-1.5"/></label><label className="text-xs font-bold text-slate-500">Akhir Semester<input type="date" value={period.akhir} onChange={(e)=>setPeriod({...period,akhir:e.target.value})} className="field mt-1.5"/></label><label className="text-xs font-bold text-slate-500">Sistem Hari Sekolah<select value={period.hariSekolah} onChange={(e)=>setPeriod({...period,hariSekolah:Number(e.target.value)})} className="field mt-1.5"><option value={5}>Senin–Jumat</option><option value={6}>Senin–Sabtu</option></select></label></div><div className="mt-3 flex flex-wrap gap-3 items-center justify-between"><p className="text-xs text-slate-400">Periode ini dipakai oleh Presensi, Perilaku, Rencana, Jurnal, dan Laporan.</p><button onClick={savePeriod} className="min-h-11 flex items-center gap-2 rounded-lg bg-emerald-600 px-3 py-2 text-xs font-bold text-white"><Save size={14}/>Simpan Periode</button></div></div><div className="rounded-2xl bg-indigo-900 p-5 text-white"><div className="text-xs font-bold uppercase tracking-wider text-emerald-300">Hari Efektif Belajar</div><div className="mt-4 text-4xl font-extrabold">{effectiveDays}</div><div className="mt-1 text-xs text-indigo-200">hari sekolah aktif dalam semester ini</div></div></div>
      ...
      {view==='daftar' && <div className="space-y-3">
        ...
        {data.filter(inPeriod).length === 0 && <p className="text-sm text-center py-8 text-gray-400">Belum ada agenda pada semester ini</p>}
      </div>}
