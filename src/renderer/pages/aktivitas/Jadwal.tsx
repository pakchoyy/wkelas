...
export default function Jadwal() {
  ...
  const [preset, setPreset] = useState({start:'07:00',breakAfter:3,breakMinutes:15})
  const [menitJP, setMenitJP] = useState(40)
  const settingsLock = useRef(false)
  const [settingsBusy, setSettingsBusy] = useState(false)
  const [settingsError, setSettingsError] = useState('')
  const [settingsDraft, setSettingsDraft] = useState({hariSekolah:5,jumlahJam:10,menitJP:40})
  ...
  const openSettings = () => {
    ...
    // Baris istirahat lama yang belum punya jam: isi jam umum (09.00 & 12.00).
    const sorted = [...istirahat].sort((a,b) => a - b)
    const backfill: Record<number,{mulai:string;selesai:string}> = {}
    sorted.forEach((jam, index) => { if (!waktuJam[jam] && ISTIRAHAT_UMUM[index]) backfill[jam] = {...ISTIRAHAT_UMUM[index]} })
  ...
  const saveSettings = async () => {
    ...
    try {
      ...
      if (!Number.isInteger(settingsDraft.menitJP) || settingsDraft.menitJP < 10 || settingsDraft.menitJP > 90) throw new Error('Menit per JP 10–90 menit.')
      const timing = usePreset ? schedulePreset(settingsDraft.jumlahJam, preset.start, settingsDraft.menitJP, preset.breakAfter, preset.breakMinutes) : null
    ...
    try {
      ...
      rows.forEach((row,index) => {
        ...
        try {
          ...
          const time = resolveScheduleTime(jam,waktuJam,data,menitJP,istirahat)
