...
import { addRecommendedSubjects } from '../../../lib/subject-storage'
...
export default function MataPelajaran() {
  ...
  const addPreset = async () => {
    ...
    try {
      const count = await addRecommendedSubjects(db, kelasId, tingkat)
      await load(); setToast(count ? `${count} mapel rekomendasi ditambahkan` : 'Tidak ada mapel baru: nama atau kode rekomendasi sudah digunakan, termasuk mapel nonaktif.')
    } catch { setError('Rekomendasi gagal diproses. Coba lagi; mapel yang sudah ada tidak ditimpa.') }
  ...
  const toggleActive = async (item: any) => {
    ...
    busyRef.current = true; setWorking(true); setError('')
    try {
      const count = await db.mata_pelajaran.update(item.id, { is_aktif: item.is_aktif === 0 ? 1 : 0 })
      if (!count) throw new Error('Mata pelajaran tidak ditemukan. Muat ulang halaman.')
      await load(); setToast(item.is_aktif === 0 ? 'Mata pelajaran diaktifkan' : 'Mata pelajaran dinonaktifkan')
    } catch { setError('Status mapel gagal diproses. Muat ulang halaman atau coba lagi.') }
    finally { busyRef.current = false; setWorking(false) }
  }
  const remove = async () => { if (!confirmDelete) return; const item = confirmDelete; setConfirmDelete(null); try { await window.electronAPI.mapel.delete(item.id); await load(); setToast('Mata pelajaran berhasil dihapus') } catch(error) { setDeleteError(error instanceof Error ? error.message : 'Mapel gagal dihapus. Mapel yang masih dipakai jadwal/rencana tidak dapat dihapus.') } }
  ...
  return <fieldset disabled={working || !ready} className="min-w-0 space-y-4">
    ...
    {deleteError && <div role="alert" className="rounded-xl border border-red-200 bg-white p-3 text-sm text-red-700">{deleteError}<button onClick={() => setDeleteError('')} className="ml-2 text-xs underline">Tutup</button></div>}
    ...
    <ConfirmDialog open={!!confirmDelete} title="Hapus mata pelajaran?" message={confirmDelete ? `Hapus ${confirmDelete.nama}? Nilai semua periode terkait juga akan dihapus. Mapel yang masih dipakai jadwal/rencana tidak dapat dihapus.` : ''} onCancel={() => setConfirmDelete(null)} onConfirm={remove} />
  </fieldset>
}
