...
export default function MataPelajaran() {
  ...
  const save = async (event: React.FormEvent) => { event.preventDefault(); if(editId) await db.mata_pelajaran.update(editId,{...form}); else await window.electronAPI.mapel.create({ kelas_id: kelasId, ...form, urutan: data.length + 1, is_aktif:1 }); setShowForm(false); setEditId(null); setForm({ nama: '', kode: '' }); await load(); setToast(editId?'Mata pelajaran berhasil diperbarui':'Mata pelajaran berhasil ditambahkan') }
  const edit=(item:MataPelajaran)=>{setEditId(item.id);setForm({nama:item.nama,kode:item.kode||''});setShowForm(true)}
