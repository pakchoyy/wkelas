...
function JurnalKelas({kelasId}: {kelasId:number}) {
  ...
  const remove = async () => { if (!editId) return; await window.electronAPI.jurnal.delete(editId); setShowForm(false); setConfirmDelete(false); await load(); setToast({ text: 'Jurnal berhasil dihapus' }) }
  const removeAll = async () => {
    try {
      const result = await window.electronAPI.jurnal.deleteAll(kelasId)
      setConfirmDeleteAll(false)
      setDrafts({})
      await load()
      setToast({ text: `${result.count || data.length} jurnal berhasil dihapus` })
    } catch {
      setToast({ text: 'Jurnal gagal dihapus. Silakan coba lagi.', error: true })
    }
  }
