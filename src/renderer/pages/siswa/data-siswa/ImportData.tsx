...
export default function ImportData({fields,kelasId,onClose,onImported}: Props) {
  ...
  return <Modal title="Impor Data Siswa" onClose={close} maxWidth="max-w-xl">
    <p className="mb-3 text-sm text-slate-600">Data dengan NIS yang sudah ada akan dilewati. Nama sama tanpa NIS perlu diperiksa. Impor tidak menimpa siswa yang sudah tersimpan.</p>
    ...
    {!result && <div className="space-y-3">
      <div className="flex flex-wrap gap-2"><button disabled={busy} onClick={async () => {try { await downloadTemplate(fields) } catch {setError('Template gagal dibuat.')}}} className="rounded-xl border px-4 py-3 text-sm">Unduh Template</button><button disabled={busy} onClick={() => fileRef.current?.click()} className="rounded-xl border px-4 py-3 text-sm">Pilih Excel / CSV</button></div>
