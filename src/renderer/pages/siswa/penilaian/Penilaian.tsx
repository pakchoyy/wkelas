...
function PenilaianKelas({kelasId}:{kelasId:number}) {
  ...
  return <div className="max-w-[1440px] mx-auto pb-20">{saveError && <p role="alert" className="mb-3 rounded-xl bg-red-50 p-3 text-sm text-red-700">{saveError}</p>}{pending > 0 && <p role="status" className="mb-3 text-sm">Menyimpan perubahan nilai...</p>}
    ...
    {mapelList.length === 0 ? <Empty title="Belum ada mata pelajaran" text="Tambahkan mata pelajaran sebelum mulai mengisi nilai." action="Tambah Mata Pelajaran" onAction={() => setShowMapel(true)}/> : <>
      ...
      <details className="mb-3 rounded-lg border border-blue-100 bg-blue-50 px-3 text-xs text-blue-900"><summary className="min-h-11 flex items-center cursor-pointer font-semibold">Petunjuk pengisian nilai</summary><p className="pb-3">Tambah kolom harian sesuai kebutuhan. UTS/UAS tersedia otomatis. Kosong berarti belum dinilai; angka 0 tetap dihitung. Nilai disimpan saat berpindah kolom.</p></details>
      ...
      <section className="lg:hidden mb-4" aria-label="Input nilai per komponen">
        <div className="rounded-xl border border-slate-200 bg-white p-3 mb-2">
          ...
          <p className="mt-2 text-xs text-slate-500">0–100 · Kosong = belum dinilai · Simpan otomatis</p>
        ...
        <div className="space-y-3">{mobileKomponen && siswa.map(student => {
          ...
          return <article key={student.id} className="grid grid-cols-[minmax(0,1fr)_auto] gap-x-3 rounded-lg border border-slate-200 bg-white px-3 py-2">
            <h3 className="self-center text-sm font-bold break-words">{student.nama}</h3>
            <div className="flex items-center gap-3"><label className="min-w-0 flex-1 text-sm text-slate-600"><span className="sr-only">{columnLabel(mobileKomponen)}</span><input aria-label={`${mobileKomponen.label} untuk ${student.nama}`} type="number" inputMode="decimal" min="0" max="100" step=".5" value={nilaiMap[key] ?? ''} onChange={e => changeNilai(key,e.target.value)} onBlur={() => saveCell(student.id,mobileKomponen.id)} onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); e.currentTarget.blur() } }} className={`mt-1 min-h-11 w-24 rounded-lg border p-2 text-base text-center ${dirty.has(key) ? 'border-amber-400 bg-amber-50' : 'border-slate-200'}`}/></label><p className="text-sm text-slate-500">Akhir <strong className="block text-sm text-emerald-700">{average(student.id)}</strong></p></div>
            <p role="status" className="col-span-2 mt-1 text-[11px] text-slate-500">{savingKey === key ? 'Menyimpan...' : dirty.has(key) ? 'Perubahan belum tersimpan' : nilaiMap[key] == null ? 'Belum dinilai' : 'Tersimpan'}</p>
          </article>
        })}</div>
