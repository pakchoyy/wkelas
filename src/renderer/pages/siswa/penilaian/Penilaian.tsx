...
function PenilaianKelas({kelasId}:{kelasId:number}) {
  ...
  const submitKomponen = async (e: React.FormEvent) => {
    ...
    try {
      if (editKomponen) {
        await window.electronAPI.kolom.update(editKomponen.id, { mata_pelajaran_id: mapelId, label: komponenForm.label || nextDailyLabel(komponen), bobot: 1, tanggal: komponenForm.tanggal || null, catatan: komponenForm.catatan || null, urutan: editKomponen.urutan })
      } else {
        // Tambah sekaligus, bawaan 4 kolom: Harian 1–4. Label mengikuti nomor
        // (mis. Tugas 2 → Tugas 3, 4, 5) agar tidak mengetik satu per satu.
        const total = Math.min(12, Math.max(1, Math.floor(Number(jumlahKolom) || 1)))
        const used = [...komponen]
        const base = komponenForm.label.trim() || nextDailyLabel(komponen)
        const numbered = base.match(/^(.*?)(\d+)$/)
        for (let i = 0; i < total; i++) {
          let label = i === 0 ? base : numbered ? `${numbered[1]}${Number(numbered[2]) + i}` : `${base} ${i + 1}`
          if (used.some((k) => k.label === label)) label = nextDailyLabel(used)
          used.push({ label } as PenilaianKolom)
          await window.electronAPI.kolom.create({ mata_pelajaran_id: mapelId, label, bobot: 1, tanggal: komponenForm.tanggal || null, catatan: komponenForm.catatan || null, urutan: dailyCount + i + 1 })
        }
      }
      setShowKomponen(false); setEditKomponen(null); setKomponenForm({ label: '', bobot: '1', tanggal: '', catatan: '' }); setJumlahKolom(4); await loadKomponen(); setToast({ type: 'success', text: `Komponen penilaian berhasil ${editKomponen ? 'diperbarui' : 'ditambahkan'}.` })
    }
  ...
  const openKomponen = (item?: PenilaianKolom) => { setEditKomponen(item || null); setJumlahKolom(4); setKomponenForm(item ? { label: item.label, bobot: '1', tanggal: item.tanggal || '', catatan: item.catatan || '' } : { label: nextDailyLabel(komponen), bobot: '1', tanggal: '', catatan: '' }); setShowKomponen(true) }
  ...
  return <div className="max-w-[1440px] mx-auto pb-4">{saveError && <p role="alert" className="mb-3 rounded-xl bg-red-50 p-3 text-sm text-red-700">{saveError}</p>}{pending > 0 && <p role="status" className="mb-3 text-sm">Menyimpan perubahan nilai...</p>}
    ...
    {mapelList.length === 0 ? <Empty title="Belum ada mata pelajaran" text="Tambahkan mata pelajaran sebelum mulai mengisi nilai." action="Tambah Mata Pelajaran" onAction={() => setShowMapel(true)}/> : <>
      <div className="rounded-xl bg-white border border-slate-200 p-2 sm:p-3 mb-3 flex flex-col md:flex-row gap-2 md:items-center">
        <label className="relative min-w-0 md:w-80"><span className="sr-only">Pilih mata pelajaran</span><BookOpen size={16} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-emerald-700"/><select value={mapelId || ''} onChange={(event) => switchMapel(Number(event.target.value))} disabled={mapelList.length === 0} className="min-h-11 w-full appearance-none rounded-xl border border-emerald-200 bg-emerald-50 py-2 pl-9 pr-10 text-sm font-extrabold text-emerald-900 outline-none focus:border-emerald-500 focus:bg-white focus:ring-2 focus:ring-emerald-500/20">{mapelList.map((item) => <option key={item.id} value={item.id}>{item.nama}</option>)}</select><ChevronDown size={16} className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-emerald-700"/></label>
...
function Empty({ title, text, action, onAction }: { title: string; text: string; action: string; onAction: () => void }) { return <div className="rounded-2xl border border-dashed border-slate-300 bg-white py-16 text-center"><ScrollText size={36} className="mx-auto text-slate-300 mb-3"/><h2 className="font-bold text-slate-700">{title}</h2><p className="text-sm text-slate-400 mt-1 mb-4">{text}</p><button onClick={onAction} className="rounded-xl bg-emerald-600 text-white px-4 py-2.5 text-sm font-bold"><Plus size={15} className="inline mr-1"/>{action}</button></div> }
