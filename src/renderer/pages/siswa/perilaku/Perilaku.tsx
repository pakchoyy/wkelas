...
function PerilakuKelas({kelasId}: {kelasId:number}) {
  ...
  return <div className="max-w-5xl mx-auto pb-16">{loadError && <p role="alert" className="mb-3 text-red-700">{loadError}</p>}
    <div className="flex rounded-xl bg-slate-200/70 p-1 w-fit mb-4"><button aria-pressed={tab === 'hari'} onClick={() => setTab('hari')} className={`min-h-11 px-3 sm:px-4 py-2 rounded-lg text-sm font-bold ${tab === 'hari' ? 'bg-white text-emerald-700 shadow-sm' : 'text-slate-500'}`}>Catatan Hari Ini</button><button aria-pressed={tab === 'riwayat'} onClick={() => setTab('riwayat')} className={`min-h-11 px-3 sm:px-4 py-2 rounded-lg text-sm font-bold ${tab === 'riwayat' ? 'bg-white text-emerald-700 shadow-sm' : 'text-slate-500'}`}>Riwayat & Rekap</button></div>
    {tab === 'hari' && <div className="rounded-2xl bg-white border border-slate-200 p-3 mb-4">
      <div className="grid grid-cols-[44px_minmax(0,1fr)_44px] items-center gap-2">
        <button aria-label="Tanggal sebelumnya" onClick={() => setTanggal(addDays(tanggal,-1))} className="size-11 rounded-xl border grid place-items-center"><ChevronLeft size={21}/></button>
        <div className="text-center text-sm sm:text-base font-extrabold">{longDate(tanggal)}</div>
        <button aria-label="Tanggal berikutnya" disabled={tanggal >= todayISO()} onClick={() => setTanggal(addDays(tanggal,1))} className="size-11 rounded-xl border grid place-items-center disabled:opacity-35"><ChevronRight size={21}/></button>
      </div>
      <label className="mt-3 flex flex-wrap justify-center items-center gap-2 text-sm text-slate-600">Pilih tanggal<input type="date" max={todayISO()} value={tanggal} onChange={e => { if (e.target.value) setTanggal(e.target.value) }} className="field !w-auto min-w-0"/></label>
    </div>}
