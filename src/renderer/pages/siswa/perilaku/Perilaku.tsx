...
import { AlertCircle, AlertTriangle, CalendarDays, CheckCircle2, ChevronDown, ChevronLeft, ChevronRight, Heart, Pencil, Search, Sparkles, Trash2, X } from 'lucide-react'
...
function PerilakuKelas({kelasId}: {kelasId:number}) {
  ...
  const [editing, setEditing] = useState<PerilakuType | null>(null)
  ...
  const openStudent = (id: number) => { setActiveId(activeId === id ? null : id); setEditing(null); setForm({ jenis: 'positif', kategori: '', deskripsi: '', tindak_lanjut: '' }) }
  const openEdit = (record: PerilakuType) => {
    setEditing(record); setTab('hari'); setTanggal(record.tanggal); setActiveId(record.siswa_id)
    setForm({ jenis: record.jenis === 'negatif' ? 'perhatian' : 'positif', kategori: record.kategori || '', deskripsi: record.deskripsi || '', tindak_lanjut: record.tindak_lanjut || '' })
  }
  const save = async () => {
    if (!activeId || !form.kategori) { setToast({ type: 'error', text: 'Pilih kategori perilaku terlebih dahulu.' }); return }
    setSaving(true)
    try {
      const payload = { siswa_id: activeId, tanggal, jenis: form.jenis, kategori: form.kategori, deskripsi: form.deskripsi || form.kategori, tindak_lanjut: form.tindak_lanjut || undefined }
      if (editing) { await window.electronAPI.perilaku.update(editing.id, payload); setEditing(null) }
      else await window.electronAPI.perilaku.create(payload)
      await load(); setActiveId(null); setToast({ type: 'success', text: `Catatan ${nameOf(activeId)} berhasil ${editing ? 'diperbarui' : 'disimpan'}.` })
    }
  ...
  return <div className="max-w-5xl mx-auto pb-16">{loadError && <p role="alert" className="mb-3 text-red-700">{loadError}</p>}
    ...
    <div className="flex max-w-full gap-1 rounded-xl p-1 w-fit mb-4"><button aria-pressed={tab === 'hari'} onClick={() => setTab('hari')} className={`tab-mint min-h-11 px-3 sm:px-4 py-2 rounded-lg text-sm font-bold ${tab === 'hari' ? 'bg-white text-emerald-700 shadow-sm' : 'text-slate-500'}`}>Catatan Hari Ini</button><button aria-pressed={tab === 'riwayat'} onClick={() => setTab('riwayat')} className={`tab-mint min-h-11 px-3 sm:px-4 py-2 rounded-lg text-sm font-bold ${tab === 'riwayat' ? 'bg-white text-emerald-700 shadow-sm' : 'text-slate-500'}`}>Riwayat & Rekap</button></div>
...
function HistoryView({ records, siswa, search, setSearch, filter, setFilter, onDelete, onEdit }: any) {
