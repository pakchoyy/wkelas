...
function PengaturanKelas({kelasId}:{kelasId:number}) {
  ...
  useEffect(() => {
    ...
    setTab(requested === 'backup' || requested === 'kelas' || requested === 'personalisasi' ? requested : 'profil')
  ...
  const saveSettings = async (event:React.FormEvent,profile:boolean) => {
    ...
    try {
      if(profile) {
        ...
        setToast(tab === 'personalisasi' ? 'Personalisasi laporan berhasil disimpan' : 'Identitas sekolah dan guru berhasil disimpan')
      } else {
  ...
  const pickLogo = async (file?: File) => {
    ...
    try {
      ...
      setToast('Logo dipilih. Klik Simpan Personalisasi agar tersimpan.')
    } catch { setError('Logo belum berhasil dibaca. Coba pilih gambar lain.') }
  }


  const clearLogo = () => {
    setGuru({...guru,foto_url:''})
    setToast('Logo dihapus. Klik Simpan Personalisasi agar tersimpan.')
  }

  const tabs=[{id:'profil' as Tab,label:'Sekolah & Guru',icon:School},{id:'kelas' as Tab,label:'Kelas & Semester',icon:BookOpen},{id:'personalisasi' as Tab,label:'Personalisasi',icon:ImageIcon},{id:'backup' as Tab,label:'Data & Cadangan',icon:Database}]
  return <div className="mx-auto max-w-4xl space-y-4">
    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      {tab==='personalisasi'&&<form onSubmit={e=>saveSettings(e,true)}><fieldset disabled={busy || loading} className="min-w-0 space-y-5"><div><h3 className="font-extrabold">Personalisasi Laporan</h3><p className="mt-1 text-sm text-slate-500">Atur logo yang muncul di kop laporan, jurnal, dan hasil cetak/PDF.</p></div><section className="rounded-3xl border border-slate-200 bg-slate-50 p-4 sm:p-5"><div className="flex flex-col gap-5 sm:flex-row sm:items-center"><span className="grid size-24 shrink-0 place-items-center overflow-hidden rounded-2xl border border-slate-200 bg-white text-emerald-700 shadow-sm">{guru?.foto_url?<img src={guru.foto_url} alt="" className="size-full object-contain p-2"/>:<ImageIcon size={30}/>}</span><div className="min-w-0 flex-1"><h4 className="font-extrabold text-slate-800">Logo sekolah / aplikasi</h4><p className="mt-1 text-sm leading-6 text-slate-500">Gunakan logo persegi atau PNG transparan. Batas 512 KB agar cepat dibuka di HP.</p><div className="mt-4 flex flex-wrap gap-2"><button type="button" onClick={()=>logoInputRef.current?.click()} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-emerald-600 px-4 text-sm font-bold text-white shadow-sm hover:bg-emerald-700"><Upload size={16}/>Pilih Logo</button>{guru?.foto_url&&<button type="button" onClick={clearLogo} className="inline-flex min-h-11 items-center gap-2 whitespace-nowrap rounded-xl border border-red-200 bg-white px-3 text-sm font-bold text-red-600 hover:bg-red-50"><X size={16}/>Hapus</button>}</div></div></div><input ref={logoInputRef} type="file" accept="image/*" className="hidden" onChange={event=>void pickLogo(event.target.files?.[0])}/></section><button className="flex items-center gap-2 rounded-xl bg-emerald-600 px-5 py-2.5 text-sm font-bold text-white"><Save size={16}/>Simpan Personalisasi</button></fieldset></form>}
